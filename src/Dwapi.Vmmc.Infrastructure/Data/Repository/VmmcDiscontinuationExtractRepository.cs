...
{
    public class VmmcDiscontinuationExtractRepository : BaseRepository<VmmcDiscontinuationExtract,Guid>, IVmmcDiscontinuationExtractRepository{public VmmcDiscontinuationExtractRepository(VmmcContext context) : base(context){}public void Process(Guid facilityId,IEnumerable<VmmcDiscontinuationExtract> extracts){var mpi = extracts.ToList();if (mpi.Any()){mpi.ForEach(x => x.FacilityId = facilityId);CreateBulk(mpi);}}}
}