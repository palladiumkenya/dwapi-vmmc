...
{
    public class VmmcMhpeExtractRepository : BaseRepository<VmmcMhpeExtract,Guid>, IVmmcMhpeExtractRepository{public VmmcMhpeExtractRepository(VmmcContext context) : base(context){}public void Process(Guid facilityId,IEnumerable<VmmcMhpeExtract> extracts){var mpi = extracts.ToList();if (mpi.Any()){mpi.ForEach(x => x.FacilityId = facilityId);CreateBulk(mpi);}}}
}